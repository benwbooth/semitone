        ...
        (let [track (get (.getTracks (.getSequence (:sequencer state))) (:track state))]
          (.add (:track state)
                (MidiEvent.
                 ...
                 (:position state))))
        (assoc state (first notes) {:notes (rest notes)}))
      state)))
...
(defn parse-length [notes state]
  (let [length (first notes)
    ...
    (cond
      ...
      (assoc state {:key (if (< length 0) nil (:key state))
      ...
      (if-let [m (re-matches #"(-|)((?:[dwhqistxofjk]\.*)+)" (name length))]
        (let [length-map {"d" 2 "w" 1 "h" 1/2 "q" 1/4 "i" 1/8 "s" 1/16 "t" 1/32
          ...
          (assoc state {:key (if (= "-" (get m 1)) nil (:key state))
                        :length (reduce +
                                        (map #(* 4.0
                                                 (get length-map (get % 2))
                                                 (- 2.0 (/ 1.0 (reduce * (repeat (count (get % 3)) 2))))
                                                 ppq)
                                             (re-seq #"([dwhqistxofjk])(\.*)" (get m 2))))
                        :notes (rest notes)}))
        (if-let [m (re-matches #"([-=])[-=]*" (name length))]
          (assoc state {:length (* (:length state) (count (name length)))
...
(defn parse-note [notes state]
  (if (= (type (first notes)) MidiMessage)
    (assoc state {:key (first notes) :notes (rest notes)})
    (let [note (clojure.string/replace (name (first notes)) #"__[0-9]+__auto__$" "#")]
      ...
      (if-let [m (re-matches #"(&|-|)([a-gA-G])(#|##|b|bb|n|)(-|)" note)]
        (let [note-map {"C" -12 "D" -10 "E" -8 "F" -7 "G" -5 "A" -3 "B" -1
              ...
              accidental-map {"#" 1 "##" 2 "b" -1 "bb" -2 "n" 0 "" 0}
              key-pressure (= "&" (get m 1))
              key (get note-map (get m 2))
              accidental (or (get accidental-map (get m 3)) ((:key-signature state) (get m 3)))]
          (assoc state {:key-type (if key-pressure ShortMessage/POLY_PRESSURE ShortMessage/NOTE_ON)
                        :tie (cond
                               (= "-" (get m 1)) :end
                               (= "-" (get m 4)) :begin
                               :else nil)
        ...
        (if-let [m (re-matches #"cc([0-9]+)" note)]
          (assoc state {:key-type ShortMessage/CONTROL_CHANGE
                        :key (mod (Integer. (get m 1)) 128)
                        :param :cc-value
                        :notes (rest notes)})
          ;; program change
          (if-let [m (re-matches #"prog([0-9]+)" note)]
            (assoc state {:key-type ShortMessage/PROGRAM_CHANGE
                          :key (mod (Integer. (get m 1)) 128)
                          :param nil
                          :notes (rest notes)})
...
(defn compose [sequencer notes & state]
  (let [notes (if (or (vector? notes) (seq? notes)) notes (seq notes))
        state (assoc {:sequencer sequencer
                      :notes '()
                      :displacement 0
                      :attack 64
                      :release 64
      ...
      (if (not (empty? notes))
        (let [state (let [state
                          (cond
                            ...
                            (assoc (compose (first notes) state) {:notes (rest notes)})
                            (map? (first notes))
                            (parse-state (first notes) state)
                            :else
                            (let [notes (:notes state)
                                  state (parse-length notes state)
                              ...
                              (cond
                                ...
                                :else nil)
                              state))]
                      (assoc state {:position (if (vector? notes)
                                                (:position state)
...
(defn play [sequencer notes & state]
  (compose sequencer notes (or state {}))
